...
import java.util.*;
    ...
    public Map<String, Object> deleteOldFlights() {
        ...
        public ResponseEntity<?> getCategoryOffers(
                @RequestParam List<Long> categories,
                @RequestParam(defaultValue = "6") int limit
        ) {
            var offersByCategory =
                    flightService.getFeaturedOffersByCategory(
                            categories,
                            limit
                    );
            ...
            for (var entry : offersByCategory.entrySet()) {
                var category = entry.getKey();

                Map<String, Object> group = new LinkedHashMap<>();

                group.put("categoryId", category.getId());
                group.put("categoryTitle", category.getTitle());
                group.put("categoryImage", category.getImage());
                group.put("categoryPromoText", category.getPromoText());
                group.put(
                        "flights",
                        entry.getValue()
                                .stream()
